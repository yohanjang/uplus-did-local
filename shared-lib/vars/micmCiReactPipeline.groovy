...
/*
 * [ MICM ] React/Yarn 기반 Application 공통 CI pipeline
 */
...
switch (args.APPLICATION_NAME) {
  ...
  case 'service-ui':
    SOURCE_REPOSITORY_GIT_URL = 'http://18.220.186.237:9000/lgu-did/service-ui.git'

  break;

  case 'solution-ui':
    SOURCE_REPOSITORY_GIT_URL = 'http://18.220.186.237:9000/lgu-did/solution-ui.git'
...
pipeline {
  ...
  environment {
    ...
    NODEJS_ARCHIVE_RELATIVE_PATH = "${args.NODEJS_ARCHIVE_RELATIVE_PATH}"

    // Yarn 빌드시 사용할 Source Repository 내 nodeJs 압축파일 이름
    NODEJS_ARCHIVE_NAME = "${args.NODEJS_ARCHIVE_NAME}"

    // Yarn 빌드를 위한 nodeJs 임시 설치 경로
    NODE_INSTALL_FULL_PATH = "${env.WORKSPACE}/node-local"

    // Yarn 빌드 전체 Command
    YARN_BUILD_FULL_COMMAND = \
        'yarn cache clean' + '\n' \
      + 'yarn install --offline' + '\n' \
      + 'yarn run' + ' ' + "${ args.YARN_RUN_COMMAND_LIST.join(' ') }"
    ...
    // base image 의 이름
    BASE_IMAGE_NAME = "${ args.BASE_IMAGE_NAME ? args.BASE_IMAGE_NAME : 'dmicma/ubi8-httpd-24' }"

    // base image 의 버전
    BASE_IMAGE_VERSION = "${ args.BASE_IMAGE_VERSION ? args.BASE_IMAGE_VERSION : '1-274' }"
    ...
    // Yarn/Docker 빌드 후 생성된 새로운 Image 를 push 할 URL
  ...
  stages {
    ...
    stage('Pre-Work Before Yarn Build') {
      ...
      steps {
        echo "=================================================================================================================="
        echo "========++++++++ Application '${env.APPLICATION_NAME}' ::: Pre-Work Before Yarn Build ::: START ++++++++========"
        echo "=================================================================================================================="

        dir("${env.SOURCE_REPOSITORY_CLONE_FULL_PATH}") {

          /*
           * [ Step 1. ]  Yarn 빌드시 사용할 NodeJs 압축 해제
           */

          sh """
[ -d "${env.NODE_INSTALL_FULL_PATH}" ] || mkdir -p "${env.NODE_INSTALL_FULL_PATH}"

tar -xf ./"${env.NODEJS_ARCHIVE_RELATIVE_PATH}/${env.NODEJS_ARCHIVE_NAME}" \
  -C "${env.NODE_INSTALL_FULL_PATH}"

"${env.NODE_INSTALL_FULL_PATH}"/bin/node -v
"""
          /*
           * [ Step 2. ]  Yarn 빌드 전, Application .env 수정 등 필요한 작업을 해당 step 에서 처리
           *
           * - 해당 Step 은 개발/Test 용도로 사용되며 추후 도메인 등 확정시 수정 예정
           */
          script {
            switch( env.APPLICATION_NAME ) {

              case 'service-ui':

                sh """
                tee ./src/.env.development << EOF
VITE_API_BASE_URL=_TENANT_URL_
VITE_SOLUTION_API_BASE_URL=https://admin-dev-lemonaid.singlex.com
VITE_TENANT_NAME=_TENANT_NAME_
EOF

                tee ./public/env.json << EOF
{
  "tenant_domain": "http://localhost:30004",
  "tenant_name": "LG U+"
}
EOF
"""
              ...
              case 'solution-ui':
                ...
                sh """
rm ./src/.env.development

tee ./src/.env.development << EOF
# Solution Backend => 실제로는 solution-ui (Apache) 접속 URL
VITE_API_BASE_URL=http://localhost:30001
# Service Backend => 실제로는 service-ui (Apache) 접속 URL
VITE_SERVICE_API_BASE_URL=http://localhost:30004
EOF
"""
              break;
      ...
      post {
        success {
          echo '======================================================================'
          echo '========++++++++ Pre-Work Before Yarn Build ::: END ++++++++========'
          echo '======================================================================'
        }
        failure {
          echo '======================================================================'
          echo '[ ERROR ]  Pre-Work Before Yarn Build ::: Failed !!!'
          echo '======================================================================'
        }
      }   // End of post
    }   // End of 'Pre-Work Before Yarn Build' stage
    ...
    stage('Yarn Build') {
      ...
      environment {
        // Yarn 빌드에 사용할 NodeJs bin 경로를 PATH 에 추가
        PATH="${env.NODE_INSTALL_FULL_PATH}/bin:${env.PATH}"
      }

      steps {
        dir("${env.SOURCE_REPOSITORY_CLONE_FULL_PATH}") {
          echo "=================================================================================================="
          echo "========++++++++ Application '${env.APPLICATION_NAME}' ::: Yarn Build ::: START ++++++++========"
          echo "=================================================================================================="

          // Yarn Build
          sh "${env.YARN_BUILD_FULL_COMMAND}"
    ...
    stage('Pre-Work Before Docker Build') {
      ...
      steps {
        dir("${env.SOURCE_REPOSITORY_CLONE_FULL_PATH}") {
          ...
          /*
           * [ Step 1. ]  Docker 빌드 전, 각 Backend 연동 위한 Reverse Proxy 설정 추가
           *
           * - 해당 Step 은 Apcahe 설정 확정 되면 각 Application 의 소스로 뺄 예정
           */
          script {
            switch( env.APPLICATION_NAME ) {
              ...
              case 'service-ui':
                ...
                sh """
tee ./micm-backend-vhost.conf << EOF
<VirtualHost *:8080>
  ...
  <Location /api>
    ProxyPass http://service-ui-service-external-service-backend:8080/api
    ProxyPassReverse http://service-ui-service-external-service-backend:8080/api
  </Location>
...
<VirtualHost *:8080>
  ...
  <Location /api>
    ProxyPass http://solution-ui-service-external-solution-backend:8080/api
    ProxyPassReverse http://solution-ui-service-external-solution-backend:8080/api
  </Location>
...
// ubi8/httpd-24 기반 Dockerfile ::: START

          sh """
tee ./Dockerfile << EOF
FROM ${env.BASE_IMAGE_FULL_URL}

# 각 backend 연동 위한 Reverse Proxy 설정 추가
COPY ./micm-backend-vhost.conf /etc/httpd/conf.d/micm-backend-vhost.conf

# Host 에서 React 빌드 완료된 dist 내 모든 파일들을 Image 내 /var/www/html 경로로 복사
COPY ./dist/ /var/www/html/

EOF"""

// ubi8/httpd-24 기반 Dockerfile ::: END
